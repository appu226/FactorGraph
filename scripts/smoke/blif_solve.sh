...
build/out/blif_solve/blif_solve --verbosity DEBUG --under_approximating_method ExactAndAccumulate --over_approximating_method FactorGraphApprox --must_count_solutions --diff_output_path temp/simple_and.dimacs test/data/simple_and.blif | grep -v sec > temp/simple_and.out
(exit ${PIPESTATUS[0]})
